...
class Chunk(val world: World,val position: ChunkPosition) {
  ...
  var updateLCG: Int = new Random().nextInt()
    ...
    def isAirBlockWorldCord(pos: BlockWorldPos): Boolean = {getBlockWorldCord(pos)== Blocks.multiAir}

    def isAirBlockChunkCord(pos: BlockWorldPos): Boolean = {getBlockChunkCord(pos)== Blocks.multiAir}
